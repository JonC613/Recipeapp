...
test('uploads a PDF, reviews it, and saves one PDF-sourced recipe without overflow', async ({ page }) => {
  ...
  await page.route('**/api/recipes/recipe-pdf-1', async (route) => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(makeRecipe({ id: 'recipe-pdf-1', title: 'PDF pasta', source: { type: 'pdf', sourceName: 'pasta.pdf', r2ObjectKey: 'imports/test/source.pdf' } })) }))
  ...
  await expect(page.getByRole('heading', { name: 'PDF pasta' })).toBeVisible()
  await expect(page.getByText('Imported from PDF: pasta.pdf')).toBeVisible()
