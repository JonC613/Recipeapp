...
test('constrains chat citations to retrieved recipe IDs in the strict provider schema', async () => {
  const request = vi.fn(async () => Response.json({ output: [{ content: [{ type: 'output_text', text: JSON.stringify({ outcome: 'answer', answer: 'Garlic Shrimp uses shrimp.', citationIds: ['shrimp-1'] }) }] }] })) as unknown as typeof fetch
