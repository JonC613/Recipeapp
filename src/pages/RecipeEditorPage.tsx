...
export function RecipeEditorPage() {
  ...
  return <section className="recipe-page"><p className="eyebrow">Manual recipe</p><h1>{recipeId ? 'Edit recipe' : 'Add a recipe'}</h1><RecipeForm initial={recipe} onSave={async (input) => { const saved = recipeId ? await updateRecipe(recipeId, input) : await createRecipe(input); await navigate(`/recipes/${saved.id}`) }} /></section>
}
