...
export function ImportDraftSummary({ imported }: { imported: RecipeImport }) {
  ...
  return <section className="import-draft" aria-labelledby="import-draft-title">
    ...
    <p className="recipe-metadata">Source: {draft.source.type === 'url' ? <a href={draft.source.originalUrl} target="_blank" rel="noreferrer">{draft.source.originalUrl}</a> : draft.source.type === 'pdf' ? `PDF${draft.source.sourceName ? `: ${draft.source.sourceName}` : ''}` : 'Pasted recipe text'}</p>
