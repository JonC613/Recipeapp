...
type ImportRow = { id: string; source_type: 'url' | 'text' | 'pdf'; source_url: string | null; source_r2_key: string | null; source_name: string | null; raw_text: string | null; parsed_recipe_json: string | null; status: 'ready' | 'failed' | 'no_recipe'; failure_code: RecipeImportFailureCode | null; ocr_status: RecipeImport['ocrStatus'] | null; ocr_attempted_at: string | null; ocr_failure_code: RecipeImportFailureCode | null; extraction_method: RecipeImport['extractionMethod'] | null; approved_recipe_id: string | null; created_at: string }
...
export async function approveImport(db: D1Database, importId: string, recipe: NormalizedManualRecipe, favorite: boolean): Promise<{ recipe?: StoredRecipe; reason: 'missing' | 'not_ready' | 'already_approved' | 'approved' }> {
  ...
  const source = imported.sourceType === 'url' && imported.sourceUrl ? { type: 'url' as const, originalUrl: imported.sourceUrl } : imported.sourceType === 'pdf' && imported.sourceR2Key ? { type: 'pdf' as const, r2ObjectKey: imported.sourceR2Key, sourceName: imported.sourceName } : { type: 'text' as const }
